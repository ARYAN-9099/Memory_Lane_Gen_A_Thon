...
from .chatbot import chatbot_configured, generate_chat_reply
...
def register_routes(app: Flask, database: Database) -> None:
    ...
    def chat():
        """Handle chatbot messages using Gemini with ranked memory context."""
        user = _require_user()
        if not user:
            return jsonify({"error": "Unauthorized"}), 401
        ...
        user_id = user.get("id")
        reply = generate_chat_reply(msg, database, int(user_id)) if user_id else ""
        if not reply:
